...
class Users::OmniauthCallbacksController < Devise::OmniauthCallbacksController
  ...
  def azure_activedirectory_v2
    ...
    if user.present?
      sign_out_all_scopes
      flash[:success] = t 'devise.omniauth_callbacks.success', kind: 'azure_activedirectory_v2'
      sign_in_and_redirect user, event: :authentication
    else
      flash[:alert] = t 'devise.omniauth_callbacks.failure', kind: 'azure_activedirectory_v2', reason: "#{auth.info.email} is not authorized."
      redirect_to new_user_session_path
    end
  end
